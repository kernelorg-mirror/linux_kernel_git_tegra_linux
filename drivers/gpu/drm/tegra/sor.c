...
static const struct tegra_sor_hdmi_settings tegra210_sor_hdmi_defaults[] = {
	{
		...
		.tx_pu = 0x66,
		.bg_vref = 0x8,
		.drive_current = { 0x33, 0x3e, 0x3e, 0x3e },
		.preemphasis = { 0x00, 0x07, 0x07, 0x07 },
	},
};
...
static int tegra_sor_hdmi_scdc(struct tegra_sor *sor)
{
	...
	if (drm_detect_hdmi_scdc(edid)) {
		...
		if (err < 0) {
			dev_err(sor->dev, "failed to read SCDC TMDS configuration: %d\n", err);
			goto out;
		}
		...
		if (err < 0) {
			dev_err(sor->dev, "failed to write SCDC TMDS configuration: %d\n", err);
			goto out;
		}
